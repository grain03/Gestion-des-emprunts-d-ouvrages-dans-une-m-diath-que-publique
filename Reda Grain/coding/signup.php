    ...
    <main>
        <div class="content">
            <div class='content_text'>
                ...
                <p>Create a new account and start a new journey with MEDIA LIB!</p>
            </div>
            <form class='login' method='post'>
                <label for="lname">Last name</label>
                <input id='lname' name='lname' type="text"  value="<?=$_POST['username']?>">
                <label for="fname">First name</label>
                <input id='fname' name='fname' type="text" value="<?=$_POST['username']?>">
                <label for="username">Username</label>
                <input id='username' name='username' type="text" value="<?=$_POST['username']?>">
                <label for="adress">Adress</label>
                <input id='adress' name='adress' type="textarea" value="<?=$_POST['username']?>">
                <label for="city">City</label>
                <select id='city' name='city' value="<?=$_POST['username']?>">
                ...
                <input id='phone_number' name='phone_number' type="number" value="<?=$_POST['username']?>">
                <label for="cin">C.I.N</label>
                <input id='cin' name='cin' type="text" value="<?=$_POST['username']?>">
                <label for="email">Email</label>
                <input id='email' name='email' type="email" value="<?=$_POST['username']?>">
                <label for="password">Password</label>
                <input id='password' name='password' type="password" value="<?=$_POST['username']?>">
                <label for="dateofbirth">Date of Birth</label>
                <input id='dateofbirth' name='dateofbirth' type="date" value="<?=$_POST['username']?>">
                <label for="type">User Account Type</label>
                <select id='type' name='type' value="<?=$_POST['username']?>">
        ...
        for($i=0; $i < $users; $i++){
            if(preg_match("/$username/i", $user[$i]['username'])){
                $username_exist = true;
            }
            if(preg_match("/$email/i", $user[$i]['email'])){
                $email_exist = true;
            }
        }
            ...
            elseif($username_exist){
                $error_msg = 'This username is already exist, please choose diffrent username';
            }
            elseif(preg_match_all("/[&<>\|`_@}{()@~'!§£=€]/", $adress)){
                $error_msg = 'Your Adress must not contain special characters!';
            }
            elseif($city == 'default'){
                $error_msg = 'Your should choose a city!';
            }
            ...
            elseif(strlen($email) > 30){
                $error_msg = 'Your Email length is more than 30!';
            }
            elseif($email_exist){
                $error_msg = 'This email is already exist';
            }        
            ...
            elseif(!preg_match_all("/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[a-zA-Z])(?=.*[&$#!§:;.?+=@]).{8,}$/", $_POST['password'])){
                ...
                $Inser_user_data = "INSERT INTO user (username,f_name,l_name,adress, tel,cin,email, password, user_type, date_of_birth, date_open_account,penalite) VALUES('$username', '$f_name','$l_name','$adress','$phone_number', '$cin', '$email', '$password' , '$type', '$date_of_birth', '$date_open_account'  , 0)";
